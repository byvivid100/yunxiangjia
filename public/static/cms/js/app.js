...
var App = function () {
    ...
    var pageLoader = function ($mode) {
        ...
        if ($mode === 'show') {
            if ($loadingEl.length) {
                ...
                jQuery('body').prepend('<div id="loading" style="background-color: rgba(0,0,0,0.62);z-index: 99998;position: fixed;padding: 10px;border-radius: 5px;color: #FFF;left: 50%;top: 50%;">' +
                    '<div class="loading-box">' +
                    '<i class="fa fa-spinner fa-2x fa-spin text-primary"></i>' +
                    '<span class="loding-text" style="padding-left:5px;display: inline-block;">加载中...</span>' +
                    '</div></div>');
            }
        } else if ($mode === 'hide') {
            if ($loadingEl.length) {
    ...
    //ajax post
    var ajaxPost = function (url, params, type) {
        pageLoader();
        $.post(url, params, function (res) {
            pageLoader('hide');
            if (res.code) {
                toastr.success(res.msg, '消息提示');
                setTimeout(function () {
                    if (type) {
    ...
    return {
        ...
        //ajax post
        ajaxPost: function (url, params, type) {
            ajaxPost(url, params, type);
        },
    };
}();
